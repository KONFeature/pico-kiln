        ...
        match channel {
            CHANNEL_TYPE_CONTROL => {
                ...
                if cdc_header.id == self.ioctl_id {
                    if cdc_header.status != 0 {
                        // PICO-KILN PATCH (from upstream embassy main): a non-zero
                        // ioctl status must NOT panic — on a WPA3-SAE rekey/flap a
                        // re-`join()` ioctl can error, and panicking here killed
                        // Core 0 (USB + WiFi). Warn and complete the ioctl; the
                        // caller (e.g. `Control::join`) sees the failure via its
                        // event wait and retries.
            ...
            CHANNEL_TYPE_EVENT => {
                ...
                let update_link_status = match (
                    ...
                    // PICO-KILN PATCH (from upstream embassy main): DEAUTH_IND and
                    // DISASSOC_IND are AP-initiated deauth/disassoc (AP reboot,
                    // idle-station timeout). Without them the driver never marks
                    // the link down after such an event, so embassy-net's
                    // wait_link_down()/is_link_up() stay stale and no reconnect
                    // monitor can fire.
