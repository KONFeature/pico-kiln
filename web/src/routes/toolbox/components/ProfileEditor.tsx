...
import { Plus, Trash2, Download, Upload, HardDrive, FileUp, CheckCircle, AlertCircle } from 'lucide-react';
import { usePico } from '@/lib/pico/context';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import type { Profile, ProfileStep, TempUnits, ProfileStepType } from '@/lib/pico/types';
...
function calculateTrajectory(profile: Profile): Segment[] {
  ...
  for (const step of profile.steps) {
    ...
    if (step.type === 'hold') {
      ...
      const durationHours = desiredRate > 0 ? tempChange / desiredRate : tempChange / 100;
...
export function ProfileEditor() {
  ...
  const [exportMode, setExportMode] = useState<'download' | 'upload'>('download');
  ...
  const { data: picoProfileContent } = useQuery({
    ...
    enabled: importMode === 'pico' && selectedPicoProfile !== '' && client !== null,
    retry: 1,
  });
  ...
  const uploadMutation = useMutation({
    ...
    onSuccess: (data) => {
      if (data.success) {
        setUploadSuccess(`Profile uploaded successfully: ${data.filename}`);
        setUploadError(null);
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => updateProfile({ temp_units: value as TempUnits })}
              >
          ...
          <div className="space-y-3">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Load an existing profile to edit</p>
            </div>
            ...
            {importMode === 'pico' && (
              <div className="space-y-2">
                ...
                {isConfigured && isLoadingProfiles && (
                  <p className="text-sm text-muted-foreground">Loading profiles from Pico...</p>
                )}
                {isConfigured && profilesData?.success && profilesData.files.length === 0 && (
                  <p className="text-sm text-muted-foreground">No profiles found on Pico</p>
                )}
                {isConfigured && profilesData?.success && profilesData.files.length > 0 && (
                  <div className="flex gap-2">
                    <Select value={selectedPicoProfile} onValueChange={setSelectedPicoProfile}>
                    ...
                    <Button
                      ...
                      disabled={!selectedPicoProfile || !picoProfileContent?.success}
                      size="sm"
                    >
                      Import
                    </Button>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 space-y-3">
                    <div className="flex items-center gap-4">
                      ...
                      <Select
                        value={step.type}
                        onValueChange={(value) => updateStep(index, { type: value as ProfileStepType })}
                      >
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <div>
                        <Label>Target Temp (°{profile.temp_units.toUpperCase()})</Label>
                        <Input
                          type="number"
                          value={step.target_temp ?? ''}
                          onChange={(e) => updateStep(index, { target_temp: parseFloat(e.target.value) || 0 })}
                        />
                      </div>
                        ...
                        <>
                          <div>
                            ...
                            <Input
                              ...
                              onChange={(e) => updateStep(index, { desired_rate: parseFloat(e.target.value) || 100 })}
                            />
                          </div>
                          <div>
                            ...
                            <Input
                              type="number"
                              value={step.min_rate ?? ''}
                              onChange={(e) => updateStep(index, { min_rate: parseFloat(e.target.value) || undefined })}
                      ...
                      {step.type === 'hold' && (
                        <div>
                          ...
                          <Input
                            type="number"
                            value={step.duration ? (step.duration / 60).toFixed(0) : ''}
                            onChange={(e) => updateStep(index, { duration: (parseFloat(e.target.value) || 0) * 60 })}
                          />
                        </div>
                      )}
                    </div>
